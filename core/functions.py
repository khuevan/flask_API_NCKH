...
def crop_objects(img, data, path, allowed_classes,name_img, img_pathname,name_folder):
    boxes, scores, classes, num_objects = data
    class_names = read_class_names(cfg.YOLO.CLASSES)
    ...
    for i in range(num_objects):
        ...
        if class_name in allowed_classes:
            ...
            img_name = str(name_img) + '_' + class_name + '_' + str(counts[class_name]) + '.png'
            img_path = os.path.join(path, img_name)
            # save image
            cv2.imwrite(img_path, cropped_img)

            crop_path = str(img_pathname)+str(name_folder)+"/"+str(img_name)
            # print(crop_path)
            crop_img.append(crop_path)
        else:
            continue
    return crop_img
